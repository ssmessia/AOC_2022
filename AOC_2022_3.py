raw='''VdzVHmNpdVmBBCpmQLTNfTtMhMJnhFhTTf
...
# -96 if alower, -38 if upper
raw = raw.splitlines()
result = 0
for r in raw:
    s1, s2 = r[:len(r)//2], r[len(r)//2:]
    common = list(set(s1).intersection(set(s2)))[0]
    if common.islower():
...
for i in range(0, len(raw),3):
    s1,s2,s3 = raw[i], raw[i+1], raw[i+2]
    common = list(set(s1).intersection(set(s2).intersection(set(s3))))[0]
    if common.islower():
        result+=ord(common)-96
    else:
        result+=ord(common)-38
